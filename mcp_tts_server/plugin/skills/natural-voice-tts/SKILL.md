...
3. Keep doing this until the user deactivates voice — do not stop after the first message.

### The court-reporter rule (the most important rule in this skill)

Act as a **court reporter, not an actor.** Speak back **exactly what you wrote** — the same words, in the same order, with nothing added, dropped, reworded, summarized, or improvised. The spoken response and the on-screen response are one and the same text.

**Allowed** — this is natural human reading aloud, *not* changing the words:
- Pronouncing symbols and numerals the way a person naturally would from context: "Louis XVI" read as "Louis the sixteenth", "$5" as "five dollars", "e.g." as "for example", "Dr." as "doctor". You are voicing the written words, not rewriting them.
- Dropping pure markdown *formatting characters* (`**`, `#`, backticks) that would otherwise be read aloud as literal symbols.

**Forbidden** — this is exactly the failure this skill exists to prevent:
- Adding any word, sentence, transition, or aside that isn't in the written response.
- Dropping or skipping any sentence.
- Rewording, paraphrasing, condensing, summarizing, or "tightening for spoken flow".
- Producing a shorter spoken version of a longer written reply — **or** a shorter written reply than what you speak. It is one response, sent identically to both the screen and `speak()`.

### Voice ON overrides any brevity preference

If you have a standing instruction or preference to keep chat replies short, concise, or direct, it still governs *how long that single response is* — but it must never cause the visible text and the spoken text to differ. Decide the length once, based on what the user actually asked for, then send that exact text to both channels. If the user asks for a long, detailed, or "page-length" answer, the on-screen text is that full length too — do not condense the screen copy. When in doubt, both channels get the fuller text; matching the two channels always takes priority over brevity while voice is on.
...
- The TTS app handles normal punctuation and Unicode (em dashes, curly quotes, ellipses) correctly, so there is no need to strip or alter those before sending.
